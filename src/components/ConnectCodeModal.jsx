...
export default function ConnectCodeModal({ userId, onClose, onOpen }) {
	...
	async function generate() {
		...
		try {
			...
			setGenerated(value); setCode('')
		} catch (insertError) {
			setError(insertError?.message || 'Could not generate a connection code.')
		} finally {
			setBusy(false)
		}
	}
	...
	async function redeem(event) {
		...
		try {
			...
			setError(openError?.message || 'That code is invalid, expired, already used, or belongs to this account.')
		} finally {
			setBusy(false)
		}
	}
